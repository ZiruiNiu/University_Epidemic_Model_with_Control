...
%       Y: the differential of state vector
    ...
    kappa_qy = u(4,:);       % effectiveness of quarantine on infected students
    kappa_qs = u(5,:);       % effectiveness of quarantine on infected staffs
    
    % Convert effectiveness of each measure to the degree of promotion or reduction on
    % the affected model parameter
    u_p = 1 - (1 - 0.28*k_m).*(1 - 0.455*k_d);
    u_m = 0.42*k_m;
    u_e = 1 + 0.21*k_e;
    u_qy = (0.5 - param.eta_y)*kappa_qy + param.eta_y;
    u_qs = (0.5 - param.eta_s)*kappa_qs + param.eta_s;
    ...
    Y(5,:) = param.epsilon_y.*E_y - (u_qy + param.rho_y).*I_y;
    Y(6,:) = param.epsilon_s.*E_s - (u_qs + param.rho_s).*I_s;
    Y(7,:) = u_qy.*I_y - param.phi_y.*Q_y;
    Y(8,:) = u_qs.*I_s - param.phi_s.*Q_s;
