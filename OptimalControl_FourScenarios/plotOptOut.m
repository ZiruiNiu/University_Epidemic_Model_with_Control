...
%       param: the structure that contains all model parameter values
    ...
    plot(tSol, [zeros(3, numstepP1), 1/0.7*uSol(1:3,:)], 'LineWidth', 1.2);
    grid on;
    ...
    subplot(2,2,4)
    % Normalize the variables to the range [0, 1]
    uSol_quara = [param.eta_y; param.eta_s] .* uSol(4:5,:) - [param.eta_y; param.eta_s];
    uSol_quara = 2*uSol_quara ./ (1 - 2*[param.eta_y; param.eta_s]);
    plot(tSol, [zeros(2, numstepP1), uSol_quara], 'LineWidth', 1.2);
    grid on;
